...
export namespace UserGateways {
	...
	export abstract class CreateUserGateway {
		abstract cancelCreation(
			input: UserGatewaysTypes.NonAuth.ICancel,
		): Promise<void>;
		abstract create(
		...
		): Promise<UserGatewaysTypes.IBundleTokens>;
	}

	export abstract class LoginUserGateway {
		abstract throwTFA(
			input: UserGatewaysTypes.NonAuth.IThrowTFA,
		): Promise<void>;
	...
	export abstract class UpdateUserGateway {
		abstract update(
			input: UserGatewaysTypes.Auth.IUpdateUser,
		): Promise<void>;
	}
	...
	export abstract class ForgotUserPasswordGateway {
		abstract start(
			input: UserGatewaysTypes.NonAuth.IStartForgotPassFlow,
		): Promise<void>;
		abstract finish(
			input: UserGatewaysTypes.NonAuth.IEndForgotPassFlow,
		): Promise<void>;
	}
}
