...
export namespace PostGateway {
	export abstract class GetPostGateway {
		...
		abstract pagination(
			input: PostGatewayTypes.IPagination,
		): Promise<PostGatewayTypes.TPaginationReturn>;
	}
	...
	export abstract class CreatePostGateway {
		abstract create(
			input: PostGatewayTypes.Server.ICreatePost,
		): Promise<PostGatewayTypes.Server.ICreatePostReturn>;
	}

	export abstract class UpdatePostGateway {
		abstract update(
			input: PostGatewayTypes.Server.IUpdatePost,
		): Promise<void>;
	}

	export abstract class DeletePostGateway {
		abstract remove(
			input: PostGatewayTypes.Server.IDeletePost,
		): Promise<void>;
	}
}
