...
export default async function sigin(req: NextApiRequest, res: NextApiResponse) {
	const { body, method } = req;
	const result = await validateBody(body);

	if (method !== "POST" || !result) return res.status(404).end();

	const serverURL = process.env.SERVER_URL;
	await Application.httpClient
		.call({
			url: `${serverURL}/users/launch-otp`,
			method: "POST",
			headers: {
